...
export const storageDownloadInputSchema = z.object({
  ...
  withProof: z.boolean().optional().describe('Verify download with Merkle proof (default: true)'),
...
export const storageDownloadTool = {
  ...
  execute: async (args: StorageDownloadInput) => {
    ...
    try {
      ...
      // Download file
      const err = await indexer.download(args.rootHash, args.outputPath, withProof);

      if (err !== null) {
        throw new Error(`Download failed: ${err}`);
      }

      void logger.info(`Download successful! File saved to: ${args.outputPath}`);
