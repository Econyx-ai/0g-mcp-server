...
export const kvSetInputSchema = z.object({
  ...
  flowContract: z
    .string()
    .optional()
    .describe(`Flow contract address (default: ${storageConfig.flowContract})`),
});
...
export const kvSetTool = {
  ...
  execute: async (args: KvSetInput) => {
    ...
    try {
      ...
      void logger.info(
        `Setting ${args.data.length} key-value pairs in stream ${args.streamId}`,
      );
      ...
      if (nodesErr !== null || !nodes || nodes.length === 0) {
        throw new Error(
          `Failed to select nodes: ${nodesErr || "No nodes available"}`,
        );
      }
